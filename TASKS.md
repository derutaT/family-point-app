...
* T_001 ポイントの記録の整備（現在値＋イベント記録）
  - Status: Doing
  - Blocked: None
  - 現在値（残高）は保持したまま、獲得/使用をイベントとして追記記録する（振り返り用・残高は導出しない）。
    既存 `history` は流用せず新規モデルで置き換え（データ移行なし）。日替わり付与も記録対象、失効は記録しない（ADR-0003 / ADR-0011）。全機能の基盤。
* T_002 遊びセッション拡張
  - Status: Doing
  - Blocked: T_001
  - 遊びイベントに「項目名」「利用時間（ポイントを利用して実行した時間量）」を記録。新規モデル設計（T_001）に統合して実施。
* T_003 Day/Week ビューの追加
  - Status: ToDo
  - Blocked: T_001, T_002
  - 新規「きろく」タブとして、獲得View / 使用View の対称形で総量＋内訳を Day / Week 表示（spec 4.2 / ADR-0010）。比較ビューの基盤も兼ねるが、単体表示だけで価値が出る。
